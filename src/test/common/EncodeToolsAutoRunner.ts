...
import {DEFAULT_ENCODE_TOOLS_NATIVE_OPTIONS} from "../../EncodeToolsNative";
...
class EncodeToolsCustomAvailableNativeModules extends EncodeTools {
  ...
  get availableNativeModules(): AvailableNativeModules {
    return this.availableNativeModulesOverrides || {
      ...
      xxhashAddon:  this.useNative,
      cborExtract:  this.useNative,
      crypto: this.useNative
    }
  }
}
...
export abstract class HashRunnerBase<I> extends EncodeToolsAutoRunner<I, BinaryInputOutput, HashAlgorithm, EncodeTools> {
  constructor(protected native: boolean, formats: HashAlgorithm[] = Object.keys(HashAlgorithm).map(enc => (HashAlgorithm as any)[enc]), factory?: EncodeToolsFactory<EncodeTools>) {
    super(native,formats, factory);
...
export class SerializeObjectRunner extends EncodeToolsAutoRunner<any, BinaryInputOutput, SerializationFormat, EncodeTools> {
  constructor(protected native: boolean, formats: SerializationFormat[] = Object.keys(SerializationFormat).map(enc => (SerializationFormat as any)[enc])) {
...
export class CompressRunner extends EncodeToolsAutoRunner<BinaryInputOutput, BinaryInputOutput, CompressionFormat, EncodeTools> {
  constructor(protected native: boolean, formats: CompressionFormat[] = Object.keys(CompressionFormat).map(enc => (CompressionFormat as any)[enc])) {
    super(native,formats);
...
export abstract class ImageRunnerBase extends EncodeToolsAutoRunner<BinaryInputOutput, BinaryInputOutput, ImageFormat, EncodeTools> {
  constructor(protected native: boolean, formats: ImageFormat[] = Object.keys(ImageFormat).map(enc => (ImageFormat as any)[enc])) {
    super(native,formats);
...
export class ImageResizeRunner extends ImageRunnerBase {
  constructor(protected native: boolean, protected width: number = (() => { let c = Chance(); return c.integer({ min: 1, max: 1e3 }) })()) {
    super(native);
  }
...
export class ImageCropRunner extends ImageRunnerBase {
  constructor(protected native: boolean, protected width: number = (() => { let c = Chance(); return c.integer({ min: 500, max: 1e3 }) })(), protected x: number = 0) {
    super(native);
  }
  public async encode(input: BinaryEncoding, format: ImageFormat): Promise<BinaryInputOutput> {
    let enc = await this.encodeToolsFactory();
...
export class ImageConvertRunner extends ImageRunnerBase {
  constructor(protected native: boolean) {
    super(native);
  }
  public async encode(input: BinaryEncoding, format: ImageFormat): Promise<BinaryInputOutput> {
    let enc = await this.encodeToolsFactory();
...
export class ImageBrightnessRunner extends ImageRunnerBase {
  constructor(protected native: boolean, protected brightnessFactor: number = (() => { let c = Chance(); return c.floating({ min: -1, max: 1 }) })()) {
    super(native);
  }
  public async encode(input: BinaryEncoding, format: ImageFormat): Promise<BinaryInputOutput> {
    let enc = await this.encodeToolsFactory();
